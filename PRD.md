...
- 设置首页最底部以低强调文本入口提供“恢复出厂设置”，不得使用与主要操作同等醒目的红色实心/描边按钮。该入口须与导出和同步操作保持足够间距。
- 恢复出厂设置必须连续确认两次：第一次完整说明会清除本浏览器的发行增量、艺人映射、筛选和 NeoDB 同步状态；第二次作为最终确认，提示操作不可撤销并建议先导出 JSON。任一步取消都不改变数据。
- 两次确认完成后恢复初始发行目录、默认艺人身份、默认筛选和默认视图，清除发行覆盖、艺人恢复快照、NeoDB 同步/OAuth 本地状态与会话 token，并回到音乐库宫格。
...
**Given** 用户在手机或窄窗口中选择一条记录作为保留项  
**When** 候选卡片展示选中状态  
**Then** 单选控件与封面保持在左侧，标题与历史摘要使用中间弹性空间，“保留”状态位于卡片右侧且不得溢出卡片；评分优先隐藏，最小宽度下状态文字收为勾选图标。

### AC-045 恢复出厂设置

**Given** 用户位于设置页底部  
**When** 页面展示恢复入口  
**Then** 使用低强调的“恢复出厂设置”文本操作，不与 NeoDB 同步、导出或备份等正常数据操作争夺视觉注意力。

**Given** 用户点击“恢复出厂设置”  
**When** 第一次确认或最终确认被取消  
**Then** 唱片、历史、艺人映射、筛选、同步状态和当前导航均保持不变。

**Given** 用户连续通过两次确认  
**When** 恢复执行  
**Then** 清除本浏览器中的用户增量、艺人映射/恢复快照、筛选、发行覆盖和 NeoDB 同步/OAuth 状态，恢复初始目录与默认视图，并明确提示“已恢复出厂设置”。
