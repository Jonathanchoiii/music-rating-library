...
- Apple Music 暂无公开的目录动态封面下载 API。首期只允许使用已有且状态为 `CONFIRMED` / `AUTO_CONFIRMED` 的精确 Apple Music 专辑链接，通过固定、可审计的开源元数据适配器检测 Apple HLS 动态封面；禁止按标题模糊搜索；
...
- 豆瓣评分必须从当前发行已确认的 NeoDB 专辑链接出发，读取 NeoDB 公开目录 API 中的 `external_resources`，不得按标题在豆瓣模糊搜索；
- 一个 NeoDB 条目可能关联多个豆瓣介质或版本。候选必须核对主标题/译名/别名、艺人和发行日期；标题不匹配，或既无艺人证据又无精确日期证据时必须留空；
- 对跳转后的豆瓣最终 URL 去重，在证据相同的候选中优先日期精确匹配，再以评分人数作为稳定的次级选择；
- 只保存公开的 10 分制平均分、评分人数、最终豆瓣 URL、检查时间与 `NEODB_EXTERNAL_RESOURCE` 证据，不抓取用户列表、评论正文或个人资料；
- NeoDB 无豆瓣外链、豆瓣无有效分数、页面拒绝访问或版本存在争议时，不写猜测值，界面明确提示“没有找到完全匹配的公开评分”。
...
- “平台评分”的刷新按钮左侧提供添加入口，用户可粘贴 AOTY、Rate Your Music、Metacritic 或 Record Club 的精确专辑页；不支持的域名、HTTP、带账号信息的 URL 或非专辑路径不得入库；
- 添加成功后立即执行一次单张按需更新；之后点击刷新时，同时核对 NeoDB 导向的豆瓣链接和所有已保存的手动链接，但不触发全库扫描，也不在打开详情或同步时自动更新；
