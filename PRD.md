...
| `/sync` | NeoDB 只读授权、增量对比与同步状态 | Owner |
...
1. 用户从导航进入“同步”；产品不再设置含义重复的“我的”入口。
...
- 桌面端侧栏和移动端底部导航的第五项为“同步”，不设置重复的“我的”个人页。
...
- 用户输入音乐库中另一条记录的 NeoDB、Apple Music 或 Spotify 唱片链接；只允许同一平台精确链接查找，不使用标题、艺人或评论模糊匹配。
- 当前发行必须已经拥有输入链接所属平台的记录；平台不一致、链接指向当前发行、未命中或一次命中多条时停止流程并解释原因，不改变音乐库。
...
- 桌面端侧栏与移动端底部导航都提供“设置”入口；“同步”仍保持为独立入口。
- `/settings` 是可扩展的设置首页，首批包含“艺人管理”“疑似重复条目”和“数据导出”；后续唱片字段、厂牌、人物 ID、隐私与地区设置放入同一层级。
...
**Given** 用户在发行 A 的详情底部输入发行 B 的 NeoDB、Apple Music 或 Spotify 唱片链接，且 A 与 B 都有该平台的精确链接  
**When** 系统精确定位 B  
...
**Given** 输入链接平台不受支持、当前发行没有同平台链接、链接只指向当前发行、未命中或同时命中多条  
**When** 用户执行查找  
