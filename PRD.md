...
- 共享状态文件只保存白名单内的用户增量，采用原子替换写入并限制为当前用户
  ...
  不得进入共享文件。
- 首次启用共享数据层时，优先迁移已有 Web 端 `4173` 状态；如检测到历史
  `5173` 本地状态，也须允许一次性并入同一共享文件。桌面端在共享文件已经
  存在时以共享文件为准，不能用刚安装应用的空状态覆盖 Web 历史。
...
- 删除 tombstone、Listening Entry 移除和人工重复项取舍采用只增不减的保护
  合并，旧客户端不得让已删除或已判定的内容重新出现。历史端口迁移只补入
  唯一数据，不能覆盖共享文件中更新较新的同一实体字段。
...
| 本地持久化 | 基准目录与用户增量分离；Web/Mac 共用 Application Support 状态；艺人编辑有恢复快照；旧端口自动迁移 | FR-031、FR-033、AC-037～038 |
...
1. **迁移前收敛**
   ...
   - 若历史 `5173` origin 仍有独有编辑，在该端口加载带共享迁移器的版本一次，
     自动写入同一共享状态；更旧构建可继续通过“合并 JSON 备份”增量汇入；
