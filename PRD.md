...
- 艺人详情沿用发行详情的关闭习惯：点击右上角关闭按钮、按下 Escape，或点击抽屉外左侧遮罩，都返回此前的艺人索引状态；点击抽屉内部不得误关闭。抽屉顶栏不显示通用「艺人详情」标题。已核验的艺人照片或动态视觉撑满抽屉宽度、裁成 1:1（`object-fit: cover; object-position: center`，不留黑边或上下黑条）。渐变模糊只占正方形下部一条底带（固定 36% 高，实心停在 94%），上半照片尽量露出给关闭按钮反白；遮罩画在裁切框内并用更长的缓入过渡到暖白 `--bg`，底边仍完全不透明，不得露出未遮照片条或发丝分隔。艺人标题保持居左，整块身份区（姓名、统计、平台图标、提示）叠在该下部渐变上，不得再在照片下方另起一段把「我的收录」顶下去。头图渐变比例已固化进 CSS，不再提供「头图渐变」调节器。叠在照片上的艺人姓名根据标题带亮度自动反白（深底白字、浅底黑字）；别名、收录统计和素材提示落在实心暖白渐变上，始终用 `--muted-dark`，避免白字落在奶油底。不把整个抽屉切成深色主题。关闭按钮和照片左右全出血保持不动。关闭按钮叠在照片上时自动反白，叠在纯色表面时恢复深色。无照片时的封面拼贴回退仍内缩圆角，可保留细分隔，并保持深色标题。已有本机动态 MP4（或尚未刷新的遗留 WebP）时，工具行保留素材请求图标，并另用与发行详情相同的 video-album 开关在动态头图与静态图之间切换；偏好写入 `artistProfiles.media.motionEnabled`，默认开启。动态头图用 muted 循环 `<video>` 显示，CSS `scale(1.2)` 铺满，底部渐变遮住放大后的空隙。
...
- NeoDB 无豆瓣外链、用户未提供精确链接、豆瓣无有效分数、页面拒绝访问或版本存在争议时，不写猜测值；若已有缓存分数则保留，界面明确提示无法读取或“没有找到完全匹配的公开评分”。
...
- 添加成功后立即执行一次单张按需更新；之后点击刷新时，同时核对 NeoDB 导向的豆瓣链接（若无手动豆瓣链接）和所有已保存的手动链接，但不触发全库扫描，也不在打开详情或同步时自动更新；
...
Phase A 已交付的产品边界包括：展示本地确认的艺人名与别名、已核验艺人照片撑满抽屉顶部并裁成 1:1，下部用更长的渐变模糊过渡到暖白且底边不露照片（固定遮罩 36% 高、实心 94%；姓名/统计/图标叠在该渐变上，「我的收录」紧接 1:1 框下方；头图渐变已固化、无调节器；叠在照片上的姓名按亮度自动反白，别名/统计用深色低强调字；无照片时回退到本地发行封面拼贴）、按 `全部 / LP / EP / Single / 未分类` 查看“我的收录”、在列表与封面宫格间切换并按艺人记住偏好、保存探索模式偏好、依据本地共同署名展示合作关系，并在从艺人页打开发行详情后返回原艺人详情。顶栏不显示「艺人详情」；有照片时头部与「我的收录」之间不加发丝分隔；关闭按钮在照片上反白、在纯色表面保持深色。顶部保留 Apple Music、Spotify、YouTube Music 三个紧凑艺人主页入口、一个 Apple Music 艺人素材请求入口，以及在已有动态 WebP 时与发行详情相同的动态/静态切换；它们是用户确认的外链与局部操作，不是数据来源说明面板。保存精确主页链接后，对应平台图标必须立即高亮并可点击打开；缺少链接的平台图标保持低强调、禁用且不可点击，链接编辑统一由旁边独立的铅笔按钮进入。

默认不自动联网：高清艺人图与公开档案刷新、约 700–1000 字艺人介绍和 Apple Music 艺人动态视觉只能由用户对当前单个艺人主动请求；普通打开、NeoDB 同步和应用启动只读取缓存。精确探索目录（未听作品灰显待解锁）尚未提供用户入口，探索模式只保存开关偏好，不得在界面承诺刷新或核验身份后会出现未听目录。Apple Music 素材请求必须使用用户确认的精确 Artist URL，走 Apple 公开目录的 `editorialArtwork` + `editorialVideo`：静态图优先方形身份静图（`artwork` / `staticDetailSquare`），再编辑用横图 / 1400/1000px，并保留 600px 兜底（含 iTunes `artworkUrl100` 放大）；`editorialVideo` 里的 `PreviewImage`/`nonvideo`/`mv.webp` 预览静图不能当动态视频；只编码 `*.itunes.apple.com` 上的 HLS `.m3u8`。本机动态文件名要把含空格的未映射艺人 ID 清理成安全 slug，合法 HLS 不得因文件名校验失败而被显示成「地址无效」。动态视觉优先方形 `motionArtistSquare1x1`，没有方形时再取 16:9 并在本机 FFmpeg 中心裁成 1:1（禁止黑边垫幅），按 1080×1080 / 30fps / crf 18 优先压成本机 H.264 MP4（超 8 MB 先自动截短时长，再降到 1080/24、960、800，直到 ≤8 MB）；无音轨。只有完全没有可播放视频时才回退静图；专辑动态封面仍走动画 WebP。旧艺人 WebP 保留可读，只在用户点击素材刷新后换成 MP4。请求失败或平台暂无素材时保留原有专辑封面拼贴，不清空上一份成功素材。外部资料必须绑定稳定艺人身份与来源，证据不足留空，不得把同名艺人、相似艺人、同厂牌或同歌单关系混入。
...
6. 三个平台艺人主页链接、Apple 素材状态和本机素材路径按稳定 `artist_id` 写入 `artistProfiles` 共享本地增量（`shared-local-state.json` 的 `recordshelf-artist-profiles-v1`）；含空格的未映射 ID（如 `raw-doja cat`）与 URL 编码变体、映射身份 ID 读取同一份档案。打开艺人详情必须还原已经保存的链接和本机素材，不得因为增量丢失而看起来像首次拉取。Apple 艺人素材只允许单艺人手动请求，不提供同步后自动扫描或全库批量素材入口。Apple Music 艺人主页只接受用户粘贴的精确 HTTPS Artist URL；不提供设置页或链接编辑器内的专辑反查匹配，不得按姓名模糊搜索 Apple 艺人，也不得在保存链接后自动拉取封面。
...
- 主页面展示已点亮地区、已到访大洲、符合资格的艺人和已评分发行数量；已到访大洲只显示当前数量，不拼接固定的六大洲总数。默认只显示已点亮地区；「全部地区」用于判断尚未点亮状态，不为未点亮地区伪造艺人或专辑。
- 地区按亚洲、欧洲、北美洲、南美洲、非洲和大洋洲分组。大洲分组标题不展示 `AS`、`EU` 等英文代码，中文名与英文全名组成单列标题，并与下方国家卡片左边缘对齐。进入已点亮地区后，按艺人展示头像、听过唱片数、平均评分，以及每张已评分发行的真实封面、专辑名和当前评分。
- 唱片点击复用既有发行详情；关闭时必须回到原国家页。艺人点击复用既有艺人详情与稳定 `artist_id`，不创建漫游专用详情副本。

### 28.2 点亮资格与证据边界

- “听过艺人”沿用现有规则：该艺人至少一张本地发行存在当前整数评分；只有收藏、愿望、评论或无评分听歌记录时不计入漫游点亮。
- 国家/地区必须同时满足：艺人公开资料研究状态为 `READY`、至少保留一个可核验来源、`publicFacts.country` 能按完整国家名或显式别名唯一映射到标准两位地区代码。
...
- 漫游结果由现有 `releases`、`artistIdentityState` 与 `artistProfileState` 在客户端实时派生，不新增国家统计持久化字段、手工点亮开关或远端同步；评分、评论与听歌日期不离开本机。普通打开、启动和同步不得发起国家研究。「我的音乐版图」标题右侧提供 50% 默认不透明度的手动刷新图标；只有用户点击后，4173 Web 或 Mac 才会后台扫描“已有评分且已有稳定艺人身份、但尚无可靠可映射国家”的艺人，刷新期间图标旋转并显示安全进度，完成后立即刷新漫游模型。4174 本机只读页将操作交给 4173，公网只读预览不提供写入口。用户仍可显式执行本机 `npm run backfill:roam-countries -- --write` 完成同类维护：优先按确认的 MusicBrainz ID 查询，无 ID 时只接受唯一精确名称/别名结果；同名歧义、来源无国家或无法映射时必须跳过，不能猜测。
...
- 桌面头部为暖白双列信息与球体布局，移动端改为单列并完整保留球体、标签和拖动提示。主页面与国家页的球体容器统一使用透明/暖白底和单层中性灰外框，不得叠加内缩的第二层装饰框；球体环境光也使用中性暖灰，不得再用紫色铺底、紫色内框或紫色阴影染色。紫色只保留给已点亮国家标记、标签代码和交互状态。地区卡桌面最多三列、移动端单列；国家页专辑封面桌面最多六列、移动端两列。
