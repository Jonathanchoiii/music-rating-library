...
6. 三个平台艺人主页链接、Apple 素材状态和本机素材路径按稳定 `artist_id` 写入 `artistProfiles` 共享本地增量（`shared-local-state.json` 的 `recordshelf-artist-profiles-v1`）；含空格的未映射 ID（如 `raw-doja cat`）与 URL 编码变体、映射身份 ID 读取同一份档案。打开艺人详情必须还原已经保存的链接和本机素材，不得因为增量丢失而看起来像首次拉取。只允许单艺人手动请求，不提供同步后自动扫描或全库批量入口。
