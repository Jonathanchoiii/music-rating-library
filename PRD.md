...
4. 探索模式开关、后续介绍结果与刷新状态写入 Web 4173 与 Mac 共用的本地增量，不进入 Git、公开构建或演示数据；探索模式当前只切换 `explorationEnabled`，不拉取外部作品目录；
5. Phase A 不因普通打开而请求外部平台；高清艺人图、公开档案、介绍与动态视觉没有可靠数据时保持空白、专辑封面拼贴或中性占位，不自动猜测。探索空状态必须诚实写明未听目录尚未开放，不得暗示点击刷新或去「核验艺人身份」就能填入灰色待解锁作品；已收录作品只出现在「我的收录」；
