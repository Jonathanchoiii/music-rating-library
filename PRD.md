...
- 唱片架以 `750 px` 为唯一布局切换断点：视口宽度大于 `750 px` 时必须继续使用横向实体唱片架，不因侧栏隐藏、浏览器缩放或可用内容宽度提前切换；只有视口宽度小于等于 `750 px` 时，才进入纵向 Cover Flow 叠放状态。叠放状态每组最多 5 张封面，保持每张封面的可识别边缘与可点击区域；组与组之间不得保留大块空白。静止状态下同组所有唱片必须使用一致的宽高、比例和缩放，不允许随层级逐张变大或缩小。只有 hover 或键盘 focus 的当前唱片可以提升到最高层级并放大，同时在自身上下临时撑开清晰间距并显示标题、艺人和评分；其他未聚焦唱片保持同样尺寸，或在聚焦期间统一缩小，不能出现彼此不同的比例。
...
- “设置 → 更新专辑封面图”提供用户主动触发的本地维护操作。每次先处理 `coverUrl` 为空或当前客户端实际加载失败的发行，再处理仍引用远程图片、尚未写入本地私有缓存的发行；显示检查进度、成功更新数和未解决数。
- 更新请求只把已确认的公开唱片目录链接或公开目录 ID 交给对应平台，不发送评分、评论、收听时间、收藏状态、艺人映射或其他私人字段。结果图片与证据仅写入本地私有缓存及 Web/Mac 共用用户增量。
- 加载失败的旧图片不得原样重试后直接视为成功；应重新按该 Release 已确认的精确 Apple Music、Spotify、NeoDB 或 MusicBrainz 链接查找。没有精确链接或所有精确来源都失败时保持缺图，不执行 MusicBrainz 标题/艺人搜索或其他模糊匹配。
...
| 名称 | 宽度 | 行为 |
|---|---:|---|
| Mobile | `< 600 px` | 底部导航、筛选底部 sheet、3 列唱片墙、纵向叠放唱片架 |
...
**Then** 四种视图都显示同一筛选结果，URL 与用户偏好同步，刷新后仍保持；唱片架桌面端每层最多 5 张，移动端改为可操作的封面叠放。
