...
- 窄屏每组最多 5 张封面以连续 Cover Flow 方式纵向叠放，保持每张封面的可识别边缘与可点击区域；组与组之间不得保留大块空白。静止状态下同组所有唱片必须使用一致的宽高、比例和缩放，不允许随层级逐张变大或缩小。只有 hover 或键盘 focus 的当前唱片可以提升到最高层级并放大，同时在自身上下临时撑开清晰间距并显示标题、艺人和评分；其他未聚焦唱片保持同样尺寸，或在聚焦期间统一缩小，不能出现彼此不同的比例。
