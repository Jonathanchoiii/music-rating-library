...
OpenAI 单张手动更新使用 Responses API 的托管联网搜索；Gemini 使用固定的 Google Generative Language API 域名、Google Search grounding 和结构化输出。两种适配器都只持久化实际搜索来源允许列表或已确认精确平台身份中的 URL。为防止把本机密钥转发到未知服务器，MVP 不提供任意 Base URL、自定义请求头或不受信任的“OpenAI 兼容接口”；增加新提供商必须在代码中建立固定域名、认证头、联网工具、结构化输出和来源校验适配器。
