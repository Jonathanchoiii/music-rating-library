...
- 写入前生成本地私人库检查点。成功写入后，详情页以流派 chips 展示值，
  并标注 Apple Music、MusicBrainz 等精确信源。
...
时间线固定按事件的有效收听时间倒序展示：最近、最新的记录在最上方，最早、最远的记录在最下方。若缺少 `listened_at`，依次以 `rated_at`、`marked_at`、更新时间和创建时间作为仅用于排序的后备值；日期相同再用更新时间和稳定 Listening Entry ID 保证顺序一致，但不得因此改写原始日期字段。只有一条记录时，不显示左侧时间轴线和紫色节点，并收回对应缩进；达到两条及以上时才显示时间轴与每条记录的节点。
...
不被覆盖，未命中或不确定条目保持空白，详情页展示流派及精确信源。
...
| 筛选时间与完整性 | 只保留发行时间和首次/最近听过时间；不提供独立标记时间或“缺目录语言”快捷项，旧值在清洗时丢弃 | FR-007、AC-015 |
| 列表收藏状态 | 独立展示听过、想听、在听、搁置；窄屏优先保留状态 | FR-007、AC-015 |
| 流派补充 | 只允许确认的 Apple Music Album ID / MusicBrainz MBID；写入证据并在详情展示；整库请求须明确授权 | FR-007、FR-010、AC-015 |
...
- 没有已确认精确目录身份时不展示空模块。存在精确 Apple Music 或（在无 Apple 时）精确 Spotify 专辑链接但尚未读取时，提供当前单张的“获取曲目列表”按钮；已有数据时同一按钮用于用户主动更新；
...
- 当且仅当没有可用的精确 Apple Music 专辑身份时，回退到已确认的 Spotify 专辑 URL（`open.spotify.com/.../album/{22位ID}`），经 Spotify Web API Client Credentials 读取该专辑曲目；凭证来自本机 `SPOTIFY_CLIENT_ID` / `SPOTIFY_CLIENT_SECRET` 或 Application Support 私人文件，默认 `market=hk`；
...
1. 精确 Apple Music 链接可返回曲目时，详情按顺序显示标题与 `m:ss`；多碟顺序稳定且时长不足一小时不显示小时位；无 Apple 但有精确 Spotify 链接且本机已配置凭证时，同样可写入并展示曲目；
