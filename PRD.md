...
- 大封面；悬停或键盘聚焦详情英雄封面时，右下角显示 30% 不透明度的刷新按钮，点击只对当前这张发行按精确链接重新获取封面。手机不依赖 hover：保持 44 × 44 热区，字形仍为 30% 不透明度。不得给整张封面加深色玻璃遮罩，也不得把该控件放到音乐库网格或唱片架上。
- 发行标题；
- 全部艺人；
- 发行类型；
- 首发日期及其精度；
...
详情抽屉顶栏不显示通用「发行详情」标题；关闭控件留在右侧，对话框可访问名称使用发行标题。专辑介绍、曲目列表、收听时间、聆听指南、平台评分等模块标题共用 20px 字号，保持近黑与现有字重、行高节奏。发行类型与平台/动态封面图标仍在同一工具行：一行能放下时不另起标题；只有图标折到第二行时，第二行左侧显示与「发行类型」同款的「收听链接」，不得恢复常驻的独立平台行或动态封面行。
...
**Then** 右下角出现 30% 不透明度的刷新按钮（手机热区 44 × 44），点击只对当前 Release ID 调用本机 `POST /api/local-enrich-covers` 并带上 `wait` 与 `force`，按已确认的精确 Apple Music / Spotify / NeoDB / Cover Art Archive 链接重新解析，不得因本地文件已存在而空转。结果的 `coverUrl` 与来源写入 `shared-local-state.json` 的用户增量（`persistUserState` / `releaseMetadataOverrides` 或对应 `userReleases`），覆盖后的同源封面用 `coverMatchedAt` 刷新图片请求。详情氛围取色仍使用当前实际展示的封面。音乐库网格与唱片架不出现该按钮；设置页「更新专辑封面图」仍只处理空缺、加载失败和尚未本地化的远程封面。
...
- 发行详情在个人评分之外提供独立的“平台评分”模块；每个平台必须同时显示品牌标识、原平台分数、评分人数（若有）与原页面入口，不能把外部评分混入用户自己的 10 分制历史；
