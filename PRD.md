...
聆听指南位于发行详情的收听时间线之后、重复条目更正之前。它帮助用户在播放专辑时理解创作背景、声音线索、重点曲目、视觉表达、专业评价与听众回响，不创建新的内容首页或社区。
...
音乐库组合筛选必须提供“专辑聆听指南”维度：“已有指南”仅匹配本地状态为 `READY` 的条目；“待生成”匹配没有指南、资料不足或仍待核验的条目。筛选依据来自本地指南状态索引，打开筛选器时重新读取；不得根据详情页是否渲染文字进行推断。
...
- 不提供设置页批量入口，也不允许全库或同步后自动扫描。每张发行详情都必须在平台链接区与收听时间线之间显示独立、可辨认的“动态封面”功能行，不以是否已有 Apple Music 链接作为入口显示条件：尚无本地动态封面时显示“请求动态封面”按钮；缺少精确 Apple Music 链接时点击后直接展开其链接编辑并说明要求；获取成功后原位变为开关。开启时详情与支持动态展示的视图读取本机 WebP，关闭时立即回退静态封面。开关选择写入 Web 4173 与 Mac 共用状态并跨端保持；
