...
- 较强候选证据包括：Unicode 规范化后完整名字一致、OpenCC 简繁规范名一致、一个完整名字包含另一个名字、或达到保守阈值的整体编辑相似度。连续相同中文字符达到 3 个可作为提示；拉丁字符片段必须更长且覆盖较短名字的足够比例。
- 只有共同字符片段、没有其他证据的候选属于弱证据，默认折叠并标记“误判可能较高”；用户仍可主动展开，不得把它们提升为自动关联依据。
