...
- “设置 → 匹配 Apple Music 艺人主页”提供用户主动触发的链接补全：只用该艺人已确认的 Apple Music 专辑链接查询目录 `include=artists`，在本地姓名/别名精确命中且 Apple 艺人 ID 唯一时填入缺失的艺人主页。合辑、同名冲突和已有链接必须跳过；不得按艺人姓名搜索 Apple 目录，也不得在匹配成功后自动拉取艺人静图或动态视觉。艺人详情的链接编辑器可对当前艺人执行同一条精确匹配。
...
- “艺人介绍”只在用户点击局部更新按钮后联网。介绍与探索模式标题右侧都使用发行详情「聆听指南」同款 44 × 44 `ArrowClockwise` 刷新按钮（`.listening-guide-update`），不用 Sparkle，也不用带「已开启」文案的紫色胶囊；探索按钮只切换本地 `explorationEnabled`。研究任务调用本机 Codex CLI 的联网检索能力，并以稳定 `artist_id`、已确认外部 ID、精确艺人主页和已确认作品作为身份锚点；同名或锚点冲突时返回资料不足，不得猜测。界面显示“准备身份、检索资料、核验来源、撰写介绍、写入本地”等安全进度，不展示隐藏推理。成功后一次性保存 700–1000 个简体中文字符的介绍，以及可核验的出生/成立日期、中文出生/成立地（国家 + 地名；无地名只显示国家）、艺人类型、流派、团体成员、获奖、提名/入围、影视作品关系、推荐聆听、关键事实来源和警告；普通打开、NeoDB 同步和音乐库加载都不得自动请求。获奖与提名/入围必须分区并逐项写明年份、奖项、类别和对应作品；影视关系必须明确为获奖、提名、入围、演唱、创作、制作或仅用于影片。公开事实区的「流派」使用紧凑、低对比度浅灰、轻圆角的小字号 chips，按内容自然折行，不得横向溢出 690px 抽屉；顶部出生/成立日期、地区、类型和流派在正文存在时仍保留。身份区右侧铅笔与 Apple 素材刷新共用 `.icon-button`、默认描边与 `var(--text)`。
- 艺人公开资料与介绍写入按稳定 `artist_id` 区分的共享本地增量，Web 与 Mac 立即复用同一结果。更新失败保留上一次成功介绍；Codex 长文没有完成时，仍应展示并写入本次或上一份已核验的公开事实（出生/成立日期、来自、地区/国籍、艺人类型、流派、团体成员），不得把整块事实区留空，也不得用百科短摘要冒充 700–1000 字介绍；不得用用户评分、评论、收听时间或模糊同名结果补写事实。无 Codex（未安装、未登录、超时或研究失败）时回退 MusicBrainz 结构化事实，并在尚无介绍时允许写入 Wikipedia 摘录；这是可核验公开档案，不是编造，也不自动改走 Gemini / OpenAI。同名无法消歧时返回 `ARTIST_IDENTITY_AMBIGUOUS` 并保留旧资料。界面必须区分 Codex 不可用、来源不足和身份冲突，且公开事实区在提示存在时仍显示。
